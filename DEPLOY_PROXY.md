...
apt -y install wireguard-tools python3 python3-venv python3-pip git

# Firewall — UFW. ORDER MATTERS: allow SSH before enabling.
ufw allow OpenSSH
ufw allow 51821/udp comment 'wg-data from CHRs'
ufw enable
ufw status verbose
```

Expected `ufw status`: `22/tcp ALLOW`, `51821/udp ALLOW`, default deny incoming.
...
>
> 🔒 **Do NOT open 1812/1813/3799 publicly.** RADIUS and CoA travel only inside
> the WireGuard tunnel. The proxy will bind them on `10.98.0.1` (wg-data), and
> UFW's default-deny keeps them off the public interface.
...
| Log: `Packet from unknown CHR IP 10.98.0.11 — dropped` | The panel's CHR record lists only the **public** IP, but over wg-data the proxy sees the **10.98.0.11** source. The allowlist comes from the panel routing-table | in the panel, make sure the CHR node entry carries the wg-data IP the proxy actually sees (or add it to the allowed IPs). This is panel-side data, not a proxy setting |
